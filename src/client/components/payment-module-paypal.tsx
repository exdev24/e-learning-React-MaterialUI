...
export default function PaymentModulePaypal(props: Props) {
  const classes = useStyles();
  const { t } = useTranslation('checkout');

  if (props.payment.isValid) {
    return <Alert color="success">{t('braintree.paypal.authorized')}</Alert>;
  }
  ...
  return (
    <div
      style={
        props.isProcessing
          ? { pointerEvents: 'none' }
          : { cursor: 'pointer', backgroundColor: '#ffc439' }
      }
      className={classes.button}
      onClick={e => {
        e.preventDefault();
        if (props.client) {
          props.client.processPayPal();
        }
      }}
    >
